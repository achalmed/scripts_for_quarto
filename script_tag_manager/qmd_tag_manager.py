...
class QMDTagManager:
    ...
    def normalize_tag(tag: str) -> str:
        ...
        tag = "".join(
            c
            for c in unicodedata.normalize("NFD", tag)
    ...
    def process_file(
        ...
        try:
            ...
            if not current_tags:
                if (
                    tags_to_add
                    and not normalize_only
                    and not replacements
                    and not tags_to_remove
                ):
                    print(
                        f"⚠️  No hay tags en {filepath} - omitiendo archivo (no se agregan tags a archivos sin tags)"
                    )
                    return False
            ...
            if replacements:
                for old_tag, new_tag in replacements.items():
                    ...
                    if old_normalized in normalized_tags:
                        ...
                        print(
                            f"   🔄 Reemplazado: '{old_normalized}' → '{new_normalized}'"
                        )

            # Eliminar tags
            if tags_to_remove:
                tags_to_remove_normalized = [
                    self.normalize_tag(tag) for tag in tags_to_remove
                ]
            ...
            if len(normalized_tags) != len(unique_tags):
                print(
                    f"   🔍 Duplicados eliminados: {len(normalized_tags) - len(unique_tags)}"
                )
...
Ejemplos de uso:
  ...
  python qmd_tag_manager.py --normalize --recursive
    ...
    if not any([args.normalize, args.replace, args.remove, args.add]):
        parser.error(
            "Debe especificar al menos una operación: --normalize, --replace, --remove o --add"
        )
    ...
    if args.replace:
        ...
        for replacement in args.replace:
            if ":" not in replacement:
                print(
                    f"⚠️  Formato inválido para reemplazo: '{replacement}'. Use 'viejo:nuevo'"
                )
                continue
